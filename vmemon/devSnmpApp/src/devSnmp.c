...
static long read_li_snmp(struct longinRecord *pli)
{
    ...
    {/* post-process */
        ...
        else
        {
            ...
            for (; isdigit(*pValStr) == 0 && *pValStr != '\0' && *pValStr!='-'; ) ++pValStr;
            /* The longin record for snmp only handles unsigned int */
            if (pValStr && sscanf(pValStr, "%u", &s32temp))
            {
...
static long read_mbbid_snmp(struct mbbiDirectRecord *pmbbid)
{
    ...
    {/* post-process */
	    ...
	    int i = sscanf(pValStr, "%02x %02x", &u32temp, &u32temp2);
            if (pValStr && i)
            {
		if( i>=2 )
                    pmbbid->val = u32temp<<8 |u32temp2;
		else
                    pmbbid->val = u32temp<<8;
