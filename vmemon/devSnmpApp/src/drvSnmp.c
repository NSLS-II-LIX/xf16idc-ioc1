...
int snmpRequestInit(dbCommon * pRecord, const char * ioString, long snmpVersion, size_t valStrLen, int cmd, char type)
{
    ...
    if(!dbFindRecord(&dbentry, pRecord->name)) {
        if(!dbFindInfo(&dbentry, "OID")) {
            strncpy(oidStr, dbGetInfoString(&dbentry), sizeof(oidStr)-1);
            oidStr[sizeof(oidStr)-1] - '\0';
        }
    }
    dbFinishEntry(&dbentry);

    if(oidStr[0]=='\0') {
        /* take OID string from link field */
        ...
        /* take OID string from info tag */
        n = sscanf(ioString, "%s %s", peerName, community);
        if(n != 2)
        {
            printf("%s: Invalid link \"%s\"\n", pRecord->name, ioString);
            return -1;
