...
#define MBBID_ON_READ_ERROR_VALUE  -999
...
extern "C" {
  ...
  static long snmpMbbiDInit(struct mbbiDirectRecord *pmbbid);
  static long snmpMbbiDRead(struct mbbiDirectRecord *pmbbid);
...
static void realInit(initHookState);
...
// mbbiDirect
struct devSnmpMBBIDIRECT{
  long            number;
  DEVSUPFUN       report;
  DEVSUPFUN       init;
  DEVSUPFUN       init_record;
  DEVSUPFUN       ioIntInfoGet;
  DEVSUPFUN       read_mbbiD;
  DEVSUPFUN       special_linconv;
} devSnmpMbbiD = {
  6,
  NULL,
  NULL,
  (DEVSUPFUN) snmpMbbiDInit,
  NULL,
  (DEVSUPFUN) snmpMbbiDRead,
  NULL
};
epicsExportAddress(dset,devSnmpMbbiD);
...
static
void oldSnmpRegistrar()
{
  initHookRegister(&realInit);
}
epicsExportRegistrar(oldSnmpRegistrar);
...
static long snmpAiInit(struct aiRecord *pai)
{
  ...
  checkInit();
  char aiStr[] = "ai";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) pai, &pai->inp, aiStr);
  if (! pDevice) {
...
static long snmpLiInit(struct longinRecord *pli)
{
  ...
  checkInit();
  char longinStr[] = "longin";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) pli, &pli->inp, longinStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
...
static long snmpSiInit(struct stringinRecord *psi)
{
  ...
  checkInit();
  char stringinStr[] = "stringin";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) psi, &psi->inp, stringinStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
...
static long snmpMbbiDInit(struct mbbiDirectRecord *pmbbid)
{
  // mbbid.inp must be an INST_IO
  switch (pmbbid->inp.type) {
    case INST_IO:
        break;

    default :
        recGblRecordError(S_db_badField,(void *)pmbbid,
                         "devSnmpMbbiD (init_record) Illegal INP field");
        return(S_db_badField);
  }

  checkInit();
  char mbbiDStr[] = "mbbiDirect";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) pmbbid, &pmbbid->inp, mbbiDStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
                      (void *)pmbbid,"devSnmpMbbiD (init_record) bad parameters");
    return(S_db_badField);
  }
...
static long snmpMbbiDRead(struct mbbiDirectRecord *pmbbid)
{
  ...
  unsigned int new_val2;

  if (pmbbid->dpvt == NULL)
    return(epicsError);
  else
    pDevice = (devSnmp_device *) pmbbid->dpvt;

  status = -1;

  /* Force to read the 16 bit status, then convert to NOBT */
  if (pDevice->getValueString(val,sizeof(val))) {
    int i = sscanf(val, "%02x %02x", &new_val, &new_val2);
    if ((i >= 1) && (! isnan(new_val))) {
      if (i==2)
        pmbbid->val = new_val<<8 |new_val2;
      else 
        pmbbid->val = new_val<<8;

      if (pmbbid->nobt < 16)
        pmbbid->val = pmbbid->val >> (16-pmbbid->nobt);
  ...
  if (pDevice->validTimeout()) {
#ifdef MBBID_ON_READ_ERROR_VALUE
    pmbbid->val = MBBID_ON_READ_ERROR_VALUE;
#endif
    status = recGblSetSevr(pmbbid, READ_ALARM, INVALID_ALARM);
    if (status && (pmbbid->stat != READ_ALARM || pmbbid->sevr != INVALID_ALARM)) {
      if (devSnmpDebug)
        recGblRecordError(-1, (void *)pmbbid, "devSnmpMbbiD read error");
    }
    status = -1;
  }
...
static epicsStatus snmpWfInit(struct waveformRecord *pwf)
{
  ...
  checkInit();
  char waveformStr[] = "waveform";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) pwf, &pwf->inp, waveformStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
...
static long snmpAoInit(struct aoRecord *pao)
{
  ...
  checkInit();
  char aoStr[] = "ao";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) pao, &pao->out, aoStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
...
static long snmpLoInit(struct longoutRecord *plo)
{
  ...
  checkInit();
  char longoutStr[] = "longout";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) plo, &plo->out, longoutStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
...
static long snmpSoInit(struct stringoutRecord *pso)
{
  ...
  checkInit();
  char stringoutStr[] = "stringout";
  devSnmp_device *pDevice = pManager->createDevice((struct dbCommon *) pso, &pso->out, stringoutStr);
  if (! pDevice) {
    recGblRecordError(S_db_badField,
...
static char *snmpVersionString(long version)
{
  static char SNMP_VERSION_1_STR[] = "SNMP_VERSION_1";
  static char SNMP_VERSION_2c_STR[] = "SNMP_VERSION_2c";
  static char SNMP_VERSION_3_STR[] = "SNMP_VERSION_3";
  static char unknown_STR[] = "unknown";
  
  switch (version) {
    case SNMP_VERSION_1:  return(SNMP_VERSION_1_STR);
    case SNMP_VERSION_2c: return(SNMP_VERSION_2c_STR);
    case SNMP_VERSION_3:  return(SNMP_VERSION_3_STR);
...
//--------------------------------------------------------------------
int epicsSnmpInit(int param) {return 0;}
static void realInit(initHookState state)
{
      ...
      bool            *okay)
{
  ...
  if(status) {
    printf("ERROR: dbFirstRecordType :  %ld\n", status);
  }
  while(!status) {
    ...
    while(!status) {
      ...
      if (info_value) {
        if(strcmp(dbGetRecordName(pdbentry), pRec->name)==0) {
          strcpy(in_oidStr, info_value);
          break;
        }
      }
      status = dbNextRecord(pdbentry);
    }
    if(strcmp(in_oidStr, "") != 0)
        break;
  ...
  if(strcmp(in_oidStr, "") != 0) {
      int n = sscanf(instioStr, "%s %s %s %d %c",
                                in_host,
                                in_community,
                                in_mask,
                                &in_data_len,
      ...
      if (n < 4) {
          ...
          printf(" format is 'host community mask maxdatalength [settype]'\n");
          return;
      }
      // parse optional set format type
      set_type = (n >= 5) ? set_format[0] : 's';
  } else {
...
//--------------------------------------------------------------------
void snmpPollAggregate::addDevice(devSnmp_device *pDevice)
{
...
//--------------------------------------------------------------------
pointerList *snmpPollAggregate::findList(unsigned long msec)
{
...
void snmpPollAggregate::report(void)
{
  ...
  for (int ii = 0; ii < lists->count(); ii++) {
    ...
    double avgr = tot / ((double) pList->count());
